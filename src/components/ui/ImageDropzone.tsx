    ...
    <div className="flex flex-col gap-3">
      <div
        ...
        className={`rounded-2xl border-2 border-dashed p-6 text-center transition-colors ${
          dragging
            ? "border-secondary bg-secondary/5 dark:border-blue-400 dark:bg-blue-500/10"
      ...
      >
        ...
        <UploadCloud
          size={28}
          aria-hidden
          className="mx-auto text-on-surface-variant dark:text-zinc-500"
        />
        ...
        <button
          ...
          className="mt-3 inline-flex min-h-11 items-center gap-2 rounded-lg border border-outline-variant px-4 text-sm font-medium text-on-surface transition-colors hover:bg-surface-container disabled:pointer-events-none disabled:opacity-50 dark:border-zinc-700 dark:text-zinc-100 dark:hover:bg-zinc-800"
        >
          <ImagePlus size={16} />
      ...
      {value.length > 0 && (
        <ul className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">
          {value.map((src, index) => (
            <li
              key={src.slice(-48)}
              className="group relative aspect-square overflow-hidden rounded-xl border border-outline-variant bg-surface-container-low dark:border-zinc-700 dark:bg-zinc-800"
            >
              <Image
                src={src}
              ...
              <div className="absolute inset-x-0 bottom-0 flex justify-end gap-1 bg-gradient-to-t from-black/70 to-transparent p-1.5">
                {index !== 0 && (
                  <button
                    ...
                    className="flex h-9 w-9 items-center justify-center rounded-lg bg-white/85 text-zinc-800 transition-colors hover:bg-white"
                  >
                    <Star size={15} />
                  </button>
                )}
                <button
                  ...
                  className="flex h-9 w-9 items-center justify-center rounded-lg bg-white/85 text-error transition-colors hover:bg-white"
                >
                  <Trash2 size={15} />
                </button>
              </div>
