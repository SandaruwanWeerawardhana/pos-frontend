...
function rowMatches(row: PaymentSaleRow, search: string, paidBy: string): boolean {
  if (paidBy !== "all" && row.paidBy !== paidBy) return false;
  if (!search.trim()) return true;
  const needle = search.trim().toLowerCase();
  return (
    row.reference.toLowerCase().includes(needle) ||
    row.saleRef.toLowerCase().includes(needle) ||
    row.customerName.toLowerCase().includes(needle)
  );
}

export default function PaymentSalesReportPage() {
  const { money } = useSettings();
  const [preset, setPreset] = useState<RangePreset>("30d");
  const [rows, setRows] = useState<PaymentSaleRow[]>([]);
  const [search, setSearch] = useState("");
  const [paidBy, setPaidBy] = useState("all");
  const [filterOpen, setFilterOpen] = useState(false);

  function reload() {
    getPaymentSalesReport(presetToRange(preset)).then(setRows);
  }

  useEffect(() => {
    let cancelled = false;
    getPaymentSalesReport(presetToRange(preset)).then((result) => {
  ...
  const columns: DataColumn<PaymentSaleRow>[] = [
    {
      key: "date",
      header: "Date",
    ...
    { key: "customer", header: "Customer", sortValue: (r) => r.customerName, render: (r) => r.customerName },
    {
      key: "paidBy",
      header: "Paid by",
      hideOnMobile: true,
  ...
  const exportColumns: ExportColumn<PaymentSaleRow>[] = [
    { key: "date", header: "Date", value: (r) => r.date },
    { key: "reference", header: "Reference", value: (r) => r.reference },
    { key: "sale", header: "Sale", value: (r) => r.saleRef },
  ...
  const title = `Sales payments — ${rangeLabel}`;
  ...
  return (
    <div className="flex flex-col gap-6 pb-8">
      <PageHeader
        ...
        breadcrumbs={[{ label: "Reports", href: ROUTES.reports }, { label: "Sales payments" }]}
      />
      ...
      <Card className="flex flex-col gap-4">
        ...
        <div className="flex flex-wrap justify-end gap-2 border-t border-outline-variant pt-4 dark:border-zinc-800">
          ...
          <Button
            ...
            variant="outline"
            size="sm"
            onClick={() => exportExcel(`payment-sales-${preset}`, title, filtered, exportColumns)}
            disabled={filtered.length === 0}
          >
            ...
            <div className="max-w-xs">
              <Select
                ...
                options={[
                  ...
                  ...paidByOptions.map((name) => ({ value: name, label: name })),
                ]}
              />
            </div>
          </div>
