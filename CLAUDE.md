...
- `src/lib/api` — `ApiClient` interface (`client.ts`: `login`, `getProducts`, `createProduct`, `updateProduct`, `deleteProduct`, `syncOrders`, `getOrders`) implemented by two interchangeable backends:
  ...
  - `real.ts` — the real Go backend, delegating to `src/lib/services/*`.
...
- `src/lib/hooks/use-sales-feed.ts` — the sales list. Server history from `apiClient.getOrders` with this device's unsynced IndexedDB orders overlaid on top, keyed on `client_generated_id` so a sale that has just synced does not appear twice. Falls back to the full local table when the server is unreachable — offline is normal on a till, not an error state.
